...
export function CommandPalette() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-white p-4 pt-16 backdrop-blur-md">
      <div className="animate-fade-in w-full max-w-2xl overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl">
        {/* Search Input Bar */}
        <div className="relative flex items-center gap-3 border-b border-slate-200 bg-white px-4 py-3">
        ...
        <div className="max-h-[28rem] scrollbar-thin scrollbar-thumb-slate-800 overflow-y-auto p-3">
          {isLoading ? (
            <div className="flex items-center justify-center gap-2 p-6 text-center text-xs text-slate-500">
            ...
            <div className="flex flex-col gap-1">
              ...
              {results.map((r) => (
                <div
                  ...
                  className="flex cursor-pointer items-center justify-between rounded-lg border border-transparent p-2.5 text-left text-slate-900 transition-colors hover:border-slate-200 hover:bg-slate-100"
                >
                  ...
                  <span className="rounded border border-slate-200 bg-slate-100 px-1.5 py-0.5 font-mono text-[9px] font-semibold text-slate-500 uppercase">
                    {r.type}
                  </span>
                </div>
              ))}
                ...
                filteredRoutes.map((route) => {
                  ...
                  return (
                    <button
                      ...
                      className="flex w-full cursor-pointer items-center justify-between rounded-lg border border-transparent p-2.5 text-left text-slate-900 transition-colors hover:border-slate-200 hover:bg-slate-100"
                    >
                      <div className="flex items-center gap-2.5">
                        <Icon className="size-4 text-slate-500" />
                        <span className="text-xs font-medium text-slate-900">{route.label}</span>
                      </div>
            ...
            <div className="flex flex-col gap-4 p-1 text-left text-xs">
              {/* Quick Actions */}
              <div>
                <span className="mb-2 block px-1 text-[10px] font-bold tracking-wider text-slate-500 uppercase">
                  Instant Actions
                </span>
                <div className="grid grid-cols-2 gap-2">
                  <button
                    onClick={handleSyncSurveillance}
                    className="flex items-center gap-2.5 rounded-lg border border-slate-200 bg-white p-2.5 text-left transition-all hover:border-slate-900 hover:bg-slate-100"
                  >
                    <Activity className="size-4 shrink-0 text-rose-400" />
                    <div>
                      <div className="text-xs font-semibold text-slate-900">Sync Surveillance</div>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="text-[10px] text-slate-500">Open scoped ledger tab</div>
                    </div>
                  </button>
                </div>
              </div>
                    ...
                    {views.map((view) => {
                      ...
                      return (
                        <button
                          ...
                          className="flex items-center gap-2.5 rounded-lg border border-slate-200 bg-white p-2 text-left transition-all hover:border-slate-200 hover:bg-slate-100"
                        >
                ...
                <div>
                  <div className="mb-2 flex items-center justify-between px-1">
                    ...
                    <button
                      ...
                      className="rounded px-1.5 py-0.5 text-[10px] font-medium text-slate-500 transition-colors hover:text-rose-400"
                    >
                      Close all tabs
                    </button>
                  </div>
                    ...
                    {tabs.map((tab) => {
                      ...
                      return (
                        <div
                          ...
                          className={[
                            "flex cursor-pointer items-center justify-between gap-2 rounded-lg border p-2 text-left transition-all hover:bg-slate-100",
                            activeTabId === tab.id
                  ...
                  {ALL_PLATFORM_ROUTES.slice(0, 12).map((route) => {
                    ...
                    return (
                      <button
                        key={route.href}
                        onClick={() => handleSelect(route.href)}
                        className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white p-2 text-left transition-all hover:border-slate-200 hover:bg-slate-100"
                      >
                        <Icon className="size-3.5 shrink-0 text-slate-500" />
                        <span className="truncate text-xs font-medium text-slate-900">
                          {route.label}
              ...
              <div className="flex items-center justify-between border-t border-slate-200 pt-2 font-mono text-[10px] text-slate-500">
                <div className="flex items-center gap-3">
                  <span>
                    <kbd className="rounded bg-slate-100 px-1 py-0.5 text-slate-500">Ctrl+B</kbd>{" "}
                    Toggle Sidebar
                  </span>
                  <span>
                    <kbd className="rounded bg-slate-100 px-1 py-0.5 text-slate-500">Alt+N</kbd> New
                    Tab
                  </span>
                  <span>
