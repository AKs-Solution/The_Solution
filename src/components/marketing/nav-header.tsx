...
export function NavHeader() {
  ...
  return (
    <header className="sticky top-0 z-50 h-16 shrink-0 border-b border-slate-200/80 bg-white/80 backdrop-blur-md">
      <div className="flex h-16 w-full items-center justify-between px-6">
        ...
        <nav className="relative z-10 hidden items-center gap-7 text-[13px] text-slate-500 md:flex">
          {NAV.map((item) => (
            <MarketingAnchor
              key={item.href}
              href={item.href}
      ...
      {open && (
        <div className="fixed top-16 right-0 left-0 z-50 border-b border-slate-200 bg-white px-6 py-4 md:hidden">
