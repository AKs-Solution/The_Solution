    ...
    <div className="w-full space-y-6 rounded-xl border border-slate-200 bg-white p-6 font-sans text-slate-900 shadow-2xl">
      ...
      <div className="flex flex-col justify-between gap-4 border-b border-slate-200 pb-6 sm:flex-row sm:items-center">
        ...
        <form onSubmit={handleSearchSubmit} className="relative flex items-center">
          ...
          <input
            ...
            className="w-64 rounded-xl border border-slate-200 bg-slate-100 py-2 pr-4 pl-9 text-xs text-slate-900 placeholder-slate-500 transition-all focus:border-indigo-500 focus:outline-none"
          />
        </form>
      </div>
      ...
      <div className="space-y-4">
        {precedents.map((p) => (
          <div
            key={p.id}
            className="space-y-3 rounded-xl border border-slate-200 bg-slate-100 p-5 shadow-md transition-all hover:border-slate-200"
          >
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                <span className="rounded border border-slate-200 bg-slate-100 px-2 py-0.5 font-mono text-xs text-slate-500">
                  Material: {p.material}
                </span>
              </div>
              <span className="font-mono text-xs text-slate-500">{p.programContext}</span>
            </div>
            ...
            <p className="text-xs leading-relaxed text-slate-700">
              <strong className="text-slate-900">Root Cause:</strong> {p.rootCause}
            </p>

            <div className="grid grid-cols-1 gap-3 border-t border-slate-200 pt-2 text-xs md:grid-cols-2">
              <div className="space-y-1 rounded-lg border border-rose-200 bg-rose-50 p-3">
                ...
                <div className="text-slate-700">{p.invalidatedAssumption}</div>
              </div>
              ...
              <div className="space-y-1 rounded-lg border border-emerald-200 bg-emerald-50 p-3">
                ...
                <div className="text-slate-700">{p.provenCorrectiveAction}</div>
              </div>
            </div>
