...
export function EngineeringCopilotChat({ onSendQuery }: EngineeringCopilotChatProps) {
  ...
  return (
    <div className="flex h-[600px] w-full flex-col rounded-xl border border-slate-800 bg-slate-900 font-sans text-slate-100 shadow-2xl">
      {/* Header */}
      <div className="flex shrink-0 items-center justify-between border-b border-slate-800 p-4">
        <div className="flex items-center gap-2">
          <Bot className="h-5 w-5 text-indigo-400" />
          <h2 className="text-sm font-bold text-white">Engineering Copilot</h2>
            ...
            <div
              className={`max-w-[85%] space-y-3 rounded-xl p-4 text-xs leading-relaxed ${
                m.sender === "USER"
                  ...
                  : "rounded-bl-none border border-slate-700 bg-slate-800 text-slate-200 shadow-md"
              }`}
            >
              <p>{m.content}</p>
              ...
              {m.copilotData && (
                <div className="space-y-2 border-t border-slate-700/80 pt-3">
                  ...
                  {m.copilotData.evidenceHashes.length > 0 && (
                    <div className="flex items-center gap-2">
                      <span className="flex items-center gap-1 rounded border border-slate-800 bg-slate-900 px-2 py-0.5 font-mono text-[10px] text-emerald-400">
                  ...
                  {m.copilotData.reasoningChain.length > 0 && (
                    <div className="space-y-1 rounded-lg border border-slate-800 bg-slate-900/60 p-2.5">
                      <div className="flex items-center gap-1 text-[10px] font-semibold tracking-wider text-slate-400 uppercase">
      ...
      >
        <input
          ...
          className="flex-1 rounded-xl border border-slate-700 bg-slate-800 px-4 py-2.5 text-xs text-white placeholder-slate-400 transition-all focus:border-indigo-500 focus:outline-none"
        />
        <button
          type="submit"
