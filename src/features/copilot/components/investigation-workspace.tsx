        ...
        <form onSubmit={handleRun} className="flex items-center gap-2">
          <div className="relative flex items-center">
            ...
            <input
              ...
              className="w-72 rounded-xl border border-slate-700 bg-slate-800 py-2 pr-4 pl-9 text-xs text-white placeholder-slate-400 transition-all focus:border-indigo-500 focus:outline-none"
            />
          </div>
          <button
            type="submit"
      ...
      {report && (
        <div className="space-y-6">
          <div className="space-y-2 rounded-xl border border-slate-800 bg-slate-800/50 p-5">
            ...
            <p className="text-xs leading-relaxed text-slate-200">{report.executiveSummary}</p>
          </div>
          ...
          {report.evidenceHashes.length > 0 && (
            <div className="flex items-center gap-2 border-t border-slate-800 pt-2">
              <span className="text-xs font-semibold text-slate-400">Evidence Hashes:</span>
              {report.evidenceHashes.map((hash, idx) => (
                <span
                  key={idx}
                  className="flex items-center gap-1 rounded border border-slate-800 bg-slate-900 px-2 py-0.5 font-mono text-[11px] text-emerald-400"
                >
