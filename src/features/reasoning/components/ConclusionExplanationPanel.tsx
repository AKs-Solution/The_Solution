...
export function ConclusionExplanationPanel({
  ...
  if (!conclusion) {
    return (
      <div className="rounded-xl border border-slate-200 bg-white p-8 text-center text-xs text-slate-500 italic">
      ...
      >
        <div className="mb-4 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <span className="font-mono text-xs text-slate-500">Confidence Score:</span>
            <span
        ...
        <p className="text-sm leading-relaxed font-medium text-slate-900">{conclusion.statement}</p>

        {conclusion.recommendation && (
          <div className="mt-5 rounded-xl border border-slate-200 bg-white p-4 text-xs">
            <span className="mb-1 block font-bold tracking-wider text-cyan-400 uppercase">
              Review Board Recommendation:
            </span>
            <p className="leading-relaxed text-slate-700">{conclusion.recommendation}</p>
          </div>
        )}
      </div>
      ...
      <div>
        ...
        <div className="space-y-2">
          {citations.map((c) => (
            <div
              key={c.evidenceId}
              className="flex items-center justify-between rounded-xl border border-slate-200 bg-white p-3 text-xs text-slate-700"
            >
