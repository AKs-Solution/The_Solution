...
export function ConclusionExplanationPanel({
  ...
  if (!conclusion) {
    return (
      <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-8 text-center text-xs text-slate-500 italic">
    ...
    <div className="space-y-6">
      ...
      <div
        className={`rounded-2xl border p-6 shadow-xl backdrop-blur-md transition-all ${
          isSupportedByEvidence
            ? "border-emerald-500/50 bg-gradient-to-br from-slate-900 via-emerald-950/40 to-slate-900 shadow-emerald-950/30"
            : "border-rose-500/50 bg-gradient-to-br from-slate-900 via-rose-950/40 to-slate-900 shadow-rose-950/30"
        }`}
      >
        ...
        <p className="text-sm leading-relaxed font-medium text-slate-200">{conclusion.statement}</p>
        ...
        {conclusion.recommendation && (
          <div className="mt-5 rounded-xl border border-slate-800 bg-slate-950/80 p-4 text-xs">
            ...
            <p className="leading-relaxed text-slate-300">{conclusion.recommendation}</p>
          </div>
        )}
      </div>
