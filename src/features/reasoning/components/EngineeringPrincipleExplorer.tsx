  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-700/60 pb-3">
        <div>
          <h3 className="text-lg font-semibold text-slate-100">Engineering Principle Library</h3>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <input
            ...
            className="w-48 rounded-lg border border-slate-700 bg-slate-900 px-3 py-1.5 text-xs text-slate-200 focus:border-cyan-500 focus:outline-none"
          />
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        {filtered.map((pr) => (
          <div
            key={pr.code}
            className="rounded-xl border border-slate-800 bg-slate-900/60 p-4 backdrop-blur-md transition-all hover:border-slate-700"
          >
            <div className="mb-2 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <span className="font-mono text-xs text-slate-400">Category: {pr.category}</span>
                <span className="font-mono text-xs text-slate-400">v{pr.version}</span>
              </div>
            </div>

            <h4 className="text-sm font-bold text-slate-100">{pr.name}</h4>
            <p className="mt-1 text-xs leading-relaxed text-slate-300">{pr.description}</p>

            {pr.governingEquations && pr.governingEquations.length > 0 && (
              <div className="mt-3 rounded-lg border border-slate-800 bg-slate-950 p-2.5">
