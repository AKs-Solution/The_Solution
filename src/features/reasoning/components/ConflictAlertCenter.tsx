...
> = {
  ...
  LOW: { bg: "bg-slate-900/60", border: "border-slate-800", badge: "bg-slate-800 text-slate-400" },
};
            ...
            {conflicts.map((cf) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="mb-3 text-xs leading-relaxed text-slate-200">{cf.description}</p>

                  <div className="rounded-lg border border-slate-800 bg-slate-950/80 p-2.5 text-xs">
                    <span className="mb-1 block font-semibold text-cyan-300">
                      Mitigation Recommendation:
                    </span>
                    <p className="text-slate-300 italic">{cf.mitigationRecommendation}</p>
                  </div>
                </div>
              );
            })}
        ...
        {uncertainties.length === 0 ? (
          ...
          <ul className="space-y-2">
            {uncertainties.map((u, i) => (
              <li
                key={i}
                className="flex items-start gap-2 rounded-lg border border-slate-800 bg-slate-900/60 p-3 text-xs text-slate-300"
              >
