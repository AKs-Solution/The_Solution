  ...
  const renderNode = (node: AuditLineageNode, depth: number = 0) => {
    ...
    return (
      <div key={node.id} className="space-y-2" style={{ paddingLeft: `${depth * 16}px` }}>
        <div className="flex items-center justify-between rounded-lg border border-slate-800 bg-slate-800/40 p-3 transition-all hover:border-slate-700">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="flex items-center gap-1 rounded border border-slate-800 bg-slate-900 px-2 py-0.5 font-mono text-xs text-emerald-400">
        ...
        {isExpanded && hasChildren && (
          <div className="ml-4 space-y-2 border-l border-slate-800 pl-2">
  ...
  return (
    <div className="w-full space-y-6 rounded-xl border border-slate-800 bg-slate-900 p-6 font-sans text-slate-100 shadow-2xl">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-4">
        ...
        <div className="flex items-center gap-2">
          <span className="flex items-center gap-1 rounded-lg border border-slate-700 bg-slate-800 px-3 py-1.5 font-mono text-xs text-slate-400">
      ...
      <div className="flex items-center justify-between rounded-xl border border-slate-800 bg-slate-800/60 p-4">
        <div>
          <div className="text-xs font-semibold tracking-wider text-slate-400 uppercase">
            Audited Target Component
          </div>
          <div className="mt-0.5 text-base font-bold text-white">{auditView.targetEntityName}</div>
        </div>
        <div className="flex items-center gap-4 text-xs text-slate-400">
          <div>
            <strong className="text-white">{auditView.decisionReplayCount}</strong> Decisions
            Replayed
          </div>
          <div>
            <strong className="text-white">{auditView.assumptionsEvaluated}</strong> Assumptions
            Evaluated
          </div>
