...
export function DrawingRiskDashboard({ assessment }: DrawingRiskDashboardProps) {
  ...
  return (
    <div className="flex flex-col gap-6 text-zinc-100">
      ...
      <Card className="border-indigo-500/20 bg-zinc-900/80 backdrop-blur-md">
        <CardContent className="p-6">
          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-zinc-800 pb-4">
            <div className="flex items-center gap-3">
              <div className="flex size-10 items-center justify-center rounded-xl border border-indigo-500/30 bg-indigo-500/10 text-indigo-400">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <Badge className="border-indigo-500/30 bg-indigo-500/10 font-mono text-indigo-300">
                    REV {metadata.revision}
                  </Badge>
                  <Badge className="border-amber-500/30 bg-amber-500/10 font-mono text-amber-300">
            ...
            <div className="flex items-center gap-3">
              <div className="text-right">
                <div className="text-xs font-semibold text-zinc-400">OVERALL RISK SCORE</div>
          ...
          <div className="grid grid-cols-2 gap-4 pt-4 text-xs sm:grid-cols-4 lg:grid-cols-6">
            <div>
              ...
              <span className="font-semibold text-zinc-200">{metadata.finish || "None"}</span>
            </div>
            <div>
            ...
            <div>
              ...
              <span className="font-semibold text-zinc-200 uppercase">{metadata.units}</span>
            </div>
            <div>
              <span className="block text-zinc-500">Scale:</span>
              <span className="font-semibold text-zinc-200">{metadata.scale || "1:1"}</span>
            </div>
            <div>
        ...
        <Card className="border-indigo-500/30 bg-indigo-950/20">
          <CardContent className="p-5">
            <div className="mb-3 flex items-center justify-between">
              ...
              <Badge className="border-indigo-500/30 bg-indigo-500/10 font-mono text-[10px] text-indigo-300">
            ...
            <div className="mt-4 flex items-center justify-between border-t border-indigo-500/20 pt-3 text-[11px]">
              <span className="text-zinc-400">Engineering Confidence:</span>
        ...
        <Card className="border-emerald-500/30 bg-emerald-950/20">
          <CardContent className="p-5">
            <div className="mb-3 flex items-center justify-between">
              ...
              <Badge className="border-emerald-500/30 bg-emerald-500/10 font-mono text-[10px] text-emerald-300">
            ...
            <div className="mt-4 flex items-center justify-between border-t border-emerald-500/20 pt-3 text-[11px]">
              <span className="text-zinc-400">Historical Confidence:</span>
        ...
        <Card className="border-amber-500/30 bg-amber-950/20">
          <CardContent className="p-5">
            ...
            <div className="mt-4 flex items-center justify-between border-t border-amber-500/20 pt-3 text-[11px]">
              <span className="text-zinc-400">Overall Fused Confidence:</span>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {categoryBreakdown.map((cat) => (
            <div
              key={cat.category}
              className="flex flex-col justify-between gap-2 rounded-xl border border-zinc-800 bg-zinc-900/60 p-3.5"
            >
              ...
              <div className="flex items-baseline gap-2">
                <span className="text-xl font-extrabold text-white">{cat.score}</span>
        ...
        <div className="flex flex-col gap-3">
          {explainability.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="overflow-hidden rounded-xl border border-zinc-800 bg-zinc-900/90 transition-colors hover:border-zinc-700"
              >
                {/* CARD HEADER */}
                <button
                  type="button"
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="flex size-8 shrink-0 items-center justify-center rounded-lg border border-amber-500/30 bg-amber-500/10 text-amber-400">
                    ...
                    <div>
                      ...
                      <p className="mt-0.5 line-clamp-1 text-xs text-zinc-400">{item.why}</p>
                    </div>
                  </div>
                ...
                {/* EXPANDABLE CONTENT */}
                {isExpanded && (
                  <div className="flex flex-col gap-4 border-t border-zinc-800 bg-zinc-950/60 p-5 text-xs">
                    {/* WHY REASONING */}
                    <div>
                      <span className="mb-1 block font-bold tracking-wider text-indigo-400 uppercase">
                        Manufacturing Physics Reasoning (Why)
                      </span>
                      <p className="rounded-lg border border-zinc-800 bg-zinc-900/80 p-3 leading-relaxed text-zinc-200">
                        {item.why}
                      </p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                      {/* EVIDENCE */}
                      <div>
                        <span className="mb-1 block font-bold tracking-wider text-zinc-400 uppercase">
                          Extracted Evidence
                        </span>
                        <div className="rounded border border-zinc-800 bg-zinc-900 p-2.5 font-mono text-zinc-300">
                          {item.evidence}
                        </div>
                      </div>
                      ...
                      <div>
                        <span className="mb-1 block font-bold tracking-wider text-emerald-400 uppercase">
                          Historical Assessment
                        </span>
                        <div className="rounded border border-zinc-800 bg-zinc-900 p-2.5 leading-relaxed text-zinc-300">
                    ...
                    <div>
                      <span className="mb-1 block font-bold tracking-wider text-amber-400 uppercase">
                        Actionable Mitigation Recommendation
                      </span>
                      <div className="flex items-start gap-2 rounded-lg border border-amber-500/20 bg-amber-500/10 p-3 text-amber-200">
