        ...
        <div className="grid grid-cols-1 gap-8 text-left lg:grid-cols-3">
                  ...
                  <Stack gap={4}>
                    ...
                    <div className="mt-1 flex items-center gap-2">
                      <input
                        ...
                        className="size-4 rounded border-slate-200 bg-slate-50 accent-indigo-600"
                      />
                      <label htmlFor="matChange" className="text-xs text-slate-600">
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="size-4 rounded border-slate-200 bg-slate-50 accent-indigo-600"
                      />
                      <label htmlFor="loadChange" className="text-xs text-slate-600">
          ...
          <div className="flex flex-col gap-6 lg:col-span-2">
            ...
            {isLoading ? (
              ...
              <Card className="border-slate-200 bg-slate-100 p-6">
                <CardContent className="flex flex-col gap-6">
                  ...
                  <div className="flex flex-col gap-1 rounded-xl border border-slate-200 bg-slate-50 p-4 text-xs">
                    <span className="font-bold text-slate-500">DISTINGUISHING REASONING:</span>
                  ...
                  <div className="flex flex-col gap-1 rounded-xl border border-indigo-500/20 bg-indigo-500/5 p-4 text-xs">
                    ...
                    <p className="text-slate-700">{result.recommendedAction}</p>
                  </div>
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    {(result.precedentDetails || []).map((prec: any, idx: number) => (
                      <div
                        key={idx}
                        className="flex flex-col gap-1 rounded-xl border border-slate-200 bg-slate-50 p-4 text-xs"
                      >
                        <span className="font-bold text-slate-900">{prec.program}</span>
                        <p className="text-slate-500">
                          Outcome:{" "}
                          <span className="font-semibold text-slate-700">{prec.faaOutcome}</span> (
                          {prec.duration})
                        </p>
                        <p className="text-slate-500">Impact: {prec.costImpact}</p>
                      </div>
                    ))}
                  </div>
                </CardContent>
              ...
              <Card className="border-dashed border-slate-200 bg-slate-100 py-20 text-center">
                <CardContent className="p-6">
                  <ShieldCheck className="mx-auto mb-3 size-8 text-slate-700" />
                  <p className="text-sm font-semibold text-slate-500">No calculation run yet</p>
