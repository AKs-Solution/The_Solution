              ...
              <Card key={idx} className="border-slate-200 bg-slate-100 p-6">
                <CardContent className="flex flex-col gap-6">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        <h3 className="text-lg font-bold text-slate-900">{s.supplierName}</h3>
                  ...
                  <div className="flex flex-col gap-1 rounded-xl border border-amber-500/20 bg-amber-500/5 p-4 text-xs">
                    ...
                    <p className="text-sm font-semibold text-slate-700">{s.predictedEvent}</p>
                  </div>
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                      {(s.recommendedMitigation || []).map((m: any, mIdx: number) => (
                        <div
                          key={mIdx}
                          className="flex flex-col gap-2 rounded-xl border border-slate-200 bg-slate-50 p-4 text-xs"
                        >
                          <div className="flex items-center justify-between">
                          ...
                          <div className="mt-1 flex items-center gap-4 text-slate-500">
                            <span className="flex items-center gap-1">
                              <DollarSign className="size-3 text-slate-500" /> Cost: $
                              {m.cost.toLocaleString()}
                            </span>
                            <span className="flex items-center gap-1">
                              <Clock className="size-3 text-slate-500" /> Schedule Impact:{" "}
                              {m.timeImpact} days
                            </span>
                          </div>
                        </div>
